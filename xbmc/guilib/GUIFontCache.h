...
struct CGUIFontCacheEntry
{
  ...
  std::chrono::steady_clock::time_point m_lastUsedMillis;
  Value m_value;

  CGUIFontCacheEntry(const CGUIFontCache<Position, Value> &cache, const CGUIFontCacheKey<Position> &key, std::chrono::steady_clock::time_point nowMillis) :
    m_cache(cache),
    m_key(key.m_pos,
  ...
  void Assign(const CGUIFontCacheKey<Position> &key, std::chrono::steady_clock::time_point nowMillis);
};
