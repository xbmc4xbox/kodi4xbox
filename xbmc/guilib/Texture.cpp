...
#else
    CLog::Log(LOGWARNING, "{} - DDS textures are not supported: {}", __FUNCTION__, texturePath);
#endif
    return false;
  }
...
#else
    CLog::Log(LOGWARNING, "{} - Textures from XBT are not supported: {}", __FUNCTION__, texturePath);
    return false;
#endif
  }
