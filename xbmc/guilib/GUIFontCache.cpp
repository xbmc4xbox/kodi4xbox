...
class CGUIFontCacheImpl
{
  struct EntryList
  {
    ...
    void UpdateAge(HashIter it, std::chrono::steady_clock::time_point millis)
    {
...
public:
  ...
  Value &Lookup(Position &pos,
                ...
                bool scrolling,
                std::chrono::steady_clock::time_point nowMillis, bool &dirtyCache);
  void Flush();
};
...
template<class Position, class Value>
void CGUIFontCacheEntry<Position, Value>::Assign(const CGUIFontCacheKey<Position> &key, std::chrono::steady_clock::time_point nowMillis)
{
                                                  ...
                                                  std::chrono::steady_clock::time_point nowMillis, bool &dirtyCache)
{
  ...
  if (i == m_list.hashMap.end())
  {
    ...
    if (!m_list.ageMap.empty() && std::chrono::duration_cast<std::chrono::milliseconds>(nowMillis - m_list.ageMap.begin()->first) > FONT_CACHE_TIME_LIMIT)
    {
...
template CGUIFontCacheStaticValue &CGUIFontCache<CGUIFontCacheStaticPosition, CGUIFontCacheStaticValue>::Lookup(CGUIFontCacheStaticPosition &, const vecColors &, const vecText &, uint32_t, float, bool, std::chrono::steady_clock::time_point, bool &);
...
template CGUIFontCacheDynamicValue &CGUIFontCache<CGUIFontCacheDynamicPosition, CGUIFontCacheDynamicValue>::Lookup(CGUIFontCacheDynamicPosition &, const vecColors &, const vecText &, uint32_t, float, bool, std::chrono::steady_clock::time_point, bool &);
