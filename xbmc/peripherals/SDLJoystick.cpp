...
void CJoystick::Initialize()
{
  ...
  {
    CLog::Log(LOGERROR, "(Re)start joystick subsystem failed : {}",SDL_GetError());
    return;
  }
...
void CJoystick::AddJoystick(int joyIndex)
{
  ...
  {
    CLog::Log(LOGNOTICE, "Ignoring Joystick {} Axis: {} Buttons: {}: invalid device properties",
      joyName.c_str(), num_axis, num_buttons);
    return;
  }

  // get details
  CLog::Log(LOGNOTICE, "Enabled Joystick: {}", joyName.c_str());
  CLog::Log(LOGNOTICE, "Details: Total Axis: {} Total Hats: {} Total Buttons: {}",
...
{
  CLog::Log(LOGDEBUG, "Joystick with id {} removed", id);
...
void CJoystick::Update()
{
  ...
  if (m_AxisIdx >= 0)
  {
    ...
    // CLog::Log(LOGDEBUG, "Joystick {} Axis {} Amount {}", JoystickIndex(joy), axisNum + 1, m_Axes[m_AxisIdx].val);
  }
  ...
  if (hatIdx == -1 && m_HatIdx != -1)
  {
    ...
    CLog::Log(LOGDEBUG, "Joystick {} hat {} hat centered", JoystickIndex(joy), hatNum + 1);
    m_pressTicksHat = 0;
  }
  else if (hatIdx != m_HatIdx)
  {
    MapHat(hatIdx, joy, hatNum);
    CLog::Log(LOGDEBUG, "Joystick {} hat {} value {}", JoystickIndex(joy), hatNum + 1, m_HatState);
  ...
  if (buttonIdx == -1 && m_ButtonIdx != -1)
  {
    ...
    CLog::Log(LOGDEBUG, "Joystick {} button {} Up", JoystickIndex(joy), buttonNum + 1);
  ...
  else if (buttonIdx != m_ButtonIdx)
  {
    ...
    CLog::Log(LOGDEBUG, "Joystick {} button {} Down", JoystickIndex(joy), buttonNum + 1);
...
uint32_t CJoystick::TranslateButton()
{
  if (m_HatIdx != -1)
  {
    if (m_HatState == 1)
  ...
  if (m_AxisIdx != -1)
  {
    for (size_t i = 0; i < m_Axes.size(); ++i)
    {
      ...
      {
        if (m_AxisIdx == 0)
          return amount < 0 ? KEY_BUTTON_LEFT_THUMB_STICK_LEFT : KEY_BUTTON_LEFT_THUMB_STICK_RIGHT;
        if (m_AxisIdx == 1)
          return amount > 0 ? KEY_BUTTON_RIGHT_THUMB_STICK_DOWN : KEY_BUTTON_RIGHT_THUMB_STICK_UP;
        if (m_AxisIdx == 2)
          return KEY_BUTTON_LEFT_ANALOG_TRIGGER;
        if (m_AxisIdx == 3)
          return amount < 0 ? KEY_BUTTON_RIGHT_THUMB_STICK_LEFT : KEY_BUTTON_RIGHT_THUMB_STICK_RIGHT;
        if (m_AxisIdx == 4)
          return amount > 0 ? KEY_BUTTON_RIGHT_THUMB_STICK_DOWN : KEY_BUTTON_RIGHT_THUMB_STICK_UP;
        if (m_AxisIdx == 5)
          return KEY_BUTTON_RIGHT_ANALOG_TRIGGER;
      }
    }
  }
  return 0;
