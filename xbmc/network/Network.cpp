...
{
#ifdef _XBOX
  hostname = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_DEVICENAME);
#else
  char hostName[128];
...
#else
  int result = fcntl(soc, F_SETFL, fcntl(soc, F_GETFL, 0) | O_NONBLOCK);
#endif
