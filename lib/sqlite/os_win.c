...
#ifdef _XBOX
  static int osType = 1;   /* 0=unknown 1=win95 2=winNT */
  // return win95 although the xbox is a win2k
  // Doing so, we only have to simulate LockFile, UnlockFile 
  // and not additionally LockFileEx
  return osType==2;
#else
...
#ifdef _XBOX
  for(;;){
    sprintf(zBuf, "Z:\\"TEMP_FILE_PREFIX);
    j = strlen(zBuf);
...
int sqlite3OsSync(OsFile *id, int dataOnly){
  ...
  if( NtFlushBuffersFile(id->h, &ioStatus) == 0 ){
#else
...
int sqlite3OsCurrentTime(double *prNow){
  ...
  *prNow = (now + (double)ft.dwLowDateTime)/864000000000.0;
  *prNow += 2305813.5;
#ifdef SQLITE_TEST
